...
impl Spy {
    ...
    pub fn handle(&mut self, m: Move) {
        ...
        let eofb_count = Edge::all().into_iter().filter(|e| self.cube.edges.eofb(*e) == EO::Flipped).count();
        let eolr_count = Edge::all().into_iter().filter(|e| self.cube.edges.eolr(*e) == EO::Flipped).count();
        let eoud_count = Edge::all().into_iter().filter(|e| self.cube.edges.eoud(*e) == EO::Flipped).count();

        let coud_count = Corner::all().into_iter().filter(|c| self.cube.corners.coud(*c) != CO::Solved).count();
        let colr_count = Corner::all().into_iter().filter(|c| self.cube.corners.colr(*c) != CO::Solved).count();
        let cofb_count = Corner::all().into_iter().filter(|c| self.cube.corners.cofb(*c) != CO::Solved).count();
        ...
        let n_lines = 3;
        println!("\r\x1b[2Keofb: {:<4}  eolr: {:<4}  eoud: {:<4}", eofb_count, eolr_count, eoud_count);
        println!("\r\x1b[2Kdrud: {:<4}  drlr: {:<4}  drfb: {:<4}", drud, drlr, drfb);
